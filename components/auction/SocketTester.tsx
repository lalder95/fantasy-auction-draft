// components/auction/SocketTester.tsx - Renamed from MockData.tsx with fixed TypeScript errors
...
export default function SocketTester() {
  ...
  useEffect(() => {
    ...
    const createSocket = () => {
      ...
      socket.on('connect', () => {
        // Only set the socketId if it's defined
        if (socket.id) {
        ...
        addMessage(`Socket connected with ID: ${socket.id || 'unknown'}`);
